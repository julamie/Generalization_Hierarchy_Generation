...
with open('road_traffic_mafft.csv', newline='') as f:
    reader = csv.reader(f)
    data = list(reader)
...
for i in data[1::2]: # former: for i in data: (skip every line with a counter)
    variants_to_arx = []
    current_variant = []
    for j in i:
...
with open("road_traffic_for_arx_mafft.csv", "w") as f:
