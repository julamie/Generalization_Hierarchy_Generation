...
filtered_log = pm4py.filter_variants_by_coverage_percentage(df_log, 0.000007)
...
for i in variants_length:
    variant = list(i['variant']) # former: i['variant'].split(',')
    while len(variant) < 6:
        variant.append('-')
...
with open("road_traffic_for_arx_naive.csv", "w") as f:
    f.write("variant,row_1,row_2,row_3,row_4,row_5,row_6\n")
