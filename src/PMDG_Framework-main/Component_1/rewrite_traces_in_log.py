...
class RewriteLog:
    ...
    def writer(self):
        ...
        pm4py.write_xes(event_log, "coselog/coselog_anonymized_log_k_5.xes")
...
log = xes_importer.apply(str(pathlib.Path().resolve()) + "/coselog/coselog.xes")
privacy_log = pd.read_csv(str(pathlib.Path().resolve()) + "/coselog/coselog_anon_k_5.csv")

#log = xes_importer.apply(str(pathlib.Path().resolve()) + "/Road_Traffic_Fine_Management_Process.xes")
#privacy_log = pd.read_csv(str(pathlib.Path().resolve()) + "/road_traffic_anon_k_5.csv")
